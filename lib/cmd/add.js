...
const resolve = (result) => {
  ...
  if (!tpls[tplName]) {
    tpls[tplName] = {};
    tpls[tplName]['url'] = gitUrl ? gitUrl.replace(/[\u0000-\u0019]/g, '') : 'https://github.com/MapleShaw/vue-cli-seed.git'; // 过滤unicode字符
    tpls[tplName]['branch'] = branch || 'master';
    tpls[tplName]['description'] = description || '默认模版';
  } else {
    tip.fail('模板已经存在!');
    process.exit();
  };
